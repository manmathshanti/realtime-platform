  ...
  return (
    <main className="relative overflow-hidden px-4 pb-16 pt-4 sm:px-6 lg:px-8">
      ...
      <section className="relative mx-auto max-w-7xl">
        <div className="glass-panel mesh-card rounded-[2rem] px-6 py-7 sm:px-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-5 font-heading text-4xl font-semibold tracking-tight text-slate-950 sm:text-5xl">
                Real-time analytics, alerts, and dashboards in one serious workspace.
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
                Built for SaaS teams that need operational awareness, polished reporting, and live telemetry without juggling five tools.
              </p>
            </div>

            <div className="grid gap-3 rounded-[1.75rem] bg-slate-950 px-5 py-5 text-slate-50 shadow-panel sm:grid-cols-2">
              <div>
                <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Organization</div>
                <div className="mt-2 text-lg font-semibold">
                  {overviewQuery.data?.organization.name ?? "Loading workspace"}
                </div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-[0.24em] text-slate-400">Suggested deploy</div>
                <div className="mt-2 text-lg font-semibold text-sky-300">Daphne + Celery + Redis</div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {kpiMeta.map((item) => (
              <article key={item.key} className="rounded-[1.5rem] border border-slate-900/10 bg-white/80 p-5">
                ...
                <div className={`mt-3 text-3xl font-semibold ${item.tone}`}>
                  {overviewQuery.data?.kpis[item.key] ?? "--"}
                </div>
              </article>
            ))}
          </div>

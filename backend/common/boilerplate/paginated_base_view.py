...
class PaginatedBaseApiView(generics.ListAPIView):
    ...
    def success(
        self, data: T, msg: _.Optional[str] = None, code=StatusCodes().SUCCESS
    ) -> Response:
        return Response(
            {
                "success": True,
    ...
    def success_paginated(self, page=1, perPage=10, data=None) -> Response:
        ...
        if data:
            ...
            res = {"HisabData": response.data.get("results")}
            result.append(res)
        else:
